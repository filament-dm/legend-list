...
function scrollTo(ctx, params) {
  ...
  state.scrollHistory.length = 0;
  if (state.scrollingTo && state.scrollingTo.isExplicitAnchor) {
    console.log(
...
function prepareMVCP(ctx, dataChanged) {
  ...
  console.log(
    `[legend-list] prepareMVCP called: dataChanged=${dataChanged}, shouldMVCP=${shouldMVCP}, scrollTarget=${scrollTarget}, isExplicitAnchor=${(_a3 = scrollingTo == null ? void 0 : scrollingTo.isExplicitAnchor) != null ? _a3 : false}`
  );
  if (shouldMVCP) {
    if (scrollTarget !== void 0) {
      ...
      if (dataChanged) {
        ...
        const targetIndex = targetId ? indexByKey.get(targetId) : void 0;
        console.log(
...
// src/core/clearExplicitAnchor.ts
function clearExplicitAnchor(ctx) {
  const state = ctx.state;
  if (state.scrollingTo && state.scrollingTo.isExplicitAnchor) {
    console.log(
...
// src/core/invalidateItemSizes.ts
function invalidateItemSizes(ctx, indices) {
  const state = ctx.state;
  ...
  for (const index of indices) {
    ...
    minIndex = minIndex !== void 0 ? Math.min(minIndex, index) : index;
  }
  if (minIndex !== void 0) {
    const existingMin = state.minIndexSizeChanged;
    state.minIndexSizeChanged = existingMin !== void 0 ? Math.min(existingMin, minIndex) : minIndex;
    console.log(
      `[legend-list] invalidateItemSizes: minIndexSizeChanged set to ${state.minIndexSizeChanged} (was ${existingMin != null ? existingMin : "undefined"})`
    );
  }
}

// src/core/setExplicitAnchor.ts
function setExplicitAnchor(ctx, anchorIndex, viewPosition = 1) {
  var _a3;
  const state = ctx.state;
  const { props } = state;
  const { data } = props;
  if (!data || anchorIndex < 0 || anchorIndex >= data.length) {
    console.warn(
      `[legend-list] setExplicitAnchor: invalid anchorIndex ${anchorIndex} (data length: ${(_a3 = data == null ? void 0 : data.length) != null ? _a3 : 0}), skipping`
    );
    return;
  }
  ...
  state.scrollingTo = {
    ...
    // Flag to distinguish from real scroll operations
    itemSize,
    offset,
    viewPosition
  };
...
function createImperativeHandle(ctx) {
  ...
  return {
    ...
    invalidateItemSizes: (options) => {
      var _a3, _b;
      console.log(
        `[legend-list] invalidateItemSizes called: indices=[${options.indices.join(", ")}], anchor=${options.anchor ? `index ${options.anchor.index}, viewPosition ${(_a3 = options.anchor.viewPosition) != null ? _a3 : 1}` : "none"}`
      );
      if (options.anchor) {
        const viewPosition = (_b = options.anchor.viewPosition) != null ? _b : 1;
